use crate::{NsecStore, NsecStoreCreator};
...
fn create_from_data() {
    ...
    let mut store = NsecStoreCreator::new_from_data(&secret_key).unwrap();
...
fn create_from_payload_const() {
    ...
    let mut store = NsecStore::new_from_payload(&payload, &password).unwrap();
...
fn neg_create_from_payload_wrong_pw_wrong_result() {
    ...
    assert_eq!(
        store.get_npub().unwrap().to_string(),
        "npub1yr6gz5k6smgufqs83mh67ugjrjnp99wdl0xkxzlekpakx4nv88fqp75qdx"
    );
...
fn write_to_file() {
    let secret_key = <[u8; 32]>::from_hex(SECRETKEY1).unwrap();
    let store = NsecStoreCreator::new_from_data(&secret_key).unwrap();
...
fn read_from_file() {
    ...
    let password = PASSWORD1.to_owned();
    let store = NsecStore::new_from_encrypted_file(&temp_file, &password).unwrap();
...
//     let pubkey = store.get_npub().unwrap();
